...
- **broadcast ส่งให้ทุกคนที่ `riderStanding` เป็น ACTIVE ไม่กรอง presence** (#152) — ไม่มีใครในทั้ง 3 รีโปเขียน `Offline` ค่าในฐานข้อมูลแปลว่า "เคยเปิดรับสักครั้ง" ถ้าจะให้ "ปิดรับ" มีผลจริงต้องมี writer ก่อน (กระทบ `DispatcherPage` ของแอดมิน = product decision)
