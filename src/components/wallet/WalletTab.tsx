...
export const WalletTab = ({
  ...
  <div className="h-full bg-[#F9FAFB] overflow-y-auto pb-32 animate-in fade-in">
    ...
    <div className="p-6 space-y-4">
      ...
      {/* ทางเข้า /probe — ชั่วคราว ลบทิ้งเมื่อ P0 จบ
          เดิมหน้า probe ตั้งใจไม่มีลิงก์จากที่ไหนเลย "เข้าโดยพิมพ์ URL เท่านั้น"
          ซึ่งใช้ไม่ได้จริง: โหมด standalone ไม่มีแถบที่อยู่ให้พิมพ์ และ manifest
          ตั้ง `start_url: "."` ไอคอนบนจอโฮมจึงเปิดที่ `/` เสมอ แม้จะกด Add to
          Home Screen ตอนอยู่หน้า /probe ก็ตาม
          ผลคือเครื่องมือที่สร้างมาเพื่อวัด "พฤติกรรมตอนเป็น PWA" ไปไม่ถึงในโหมด
          PWA พอดี — ต้องมีลิงก์จากในแอปเท่านั้นถึงจะวัดได้ */}
      <a
        href="/probe"
        className="block text-center text-[10px] text-gray-300 pt-6 pb-2"
      >
        P0 probe (เครื่องมือวัดภายใน)
      </a>
    </div>
  </div>
);
