...
// to the job document so admin / Internal QC can audit later.
//
// This step is OPTIONAL — rider can skip and proceed straight to
// inspection. We don't gate inspection on it to keep flexibility for
// edge cases (customer's screen broken, can't open Settings, etc.),
// but Find My ON triggers a blocker modal even when skipped.
...
export const DeviceVerificationModal = ({ job, onClose, onComplete }: Props) => {
  ...
  const handleUpload = async (file: File | undefined, slot: Slot) => {
    ...
    try {
      ...
      setter((s: any) => ({ ...s, url, uploading: false, ocring: true }));
      ...
      try {
        ...
        setter((s: any) => ({ ...s, ocring: false }));
        toast.info('อ่านข้อมูลอัตโนมัติไม่ได้ — กรุณากรอกเอง');
      }
    } catch (e: any) {
      setter((s: any) => ({ ...s, uploading: false }));
  ...
  // Save what we've got to the job. Empty fields are simply omitted —
  // the rider can come back later if needed.
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-100 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">ถ่าย 3 หน้าจอจาก Settings — ระบบจะอ่านอัตโนมัติ</p>
            </div>
          </div>
        ...
        <div className="p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] border-t border-gray-100 shrink-0 space-y-2">
          ...
          >
            {isSaving ? (
              ...
              <><ShieldCheck size={20} /> บันทึกและเริ่มตรวจสภาพ</>
            )}
          </button>
          <button
            onClick={onClose}
          ...
          >
            ข้ามขั้นตอนนี้ (เครื่องลูกค้ามีปัญหา)
          </button>
        </div>
      </div>
    </div>
...
// ─────────────────────────────────────────────────────────────────────
// Slot — repeated UI for each of the 3 captures: title, instruction,
// camera button or thumbnail, OCR-result block.
// ─────────────────────────────────────────────────────────────────────
...
function Slot({ title, instruction, icon: Icon, state, onUpload, onClear, renderResult }: SlotProps) {
