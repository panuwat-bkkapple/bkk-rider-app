    ...
    <div className="fixed inset-0 bg-gray-50 z-40 overflow-y-auto animate-in fade-in duration-200">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 shadow-[0_-4px_20px_rgba(0,0,0,0.06)] z-20">
        <div className="max-w-md mx-auto">
          ...
          {mode === 'active' && job.status === 'Accepted' && (
            <div className="flex gap-2">
          ...
          {mode === 'active' && job.status === 'Heading to Customer' && (
            <button onClick={() => handleAction('arrived', () => onUpdateStatus(job.id, 'Arrived', 'ถึงจุดหมายแล้ว'))} disabled={!!loadingAction} className="w-full bg-emerald-500 text-white py-4 rounded-2xl font-bold shadow-md active:scale-95 flex justify-center items-center gap-2 disabled:opacity-50">
              {loadingAction === 'arrived' ? <Loader2 size={20} className="animate-spin" /> : <MapPin size={20} />} ถึงจุดหมายแล้ว
            </button>
          )}

          {mode === 'active' && (job.status === 'Arrived' || job.status === 'Being Inspected') && (
          ...
          {mode === 'active' && job.status === 'In-Transit' && (
