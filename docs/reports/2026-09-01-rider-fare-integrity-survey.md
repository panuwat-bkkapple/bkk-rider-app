...
>
> **ขนาดของผลกระทบ: สแกนเจอ 50 ใบ แต่ 49 ใบเป็นงานทดสอบของเจ้าของเอง หนี้จริง
> กับคนอื่นคือ 25 บาทใบเดียว และจ่ายไปแล้ว** — อย่าอ่านยอดรวม 1,366 บาทว่าเป็นเงิน
> ค้างจ่ายไรเดอร์ ดูหัวข้อ "ขนาดของผลกระทบจริง" ก่อนเอาตัวเลขใดไปตั้งเป็นงาน
...
### สถานะปัจจุบัน — ปิดแล้ว (2 ก.ย. 2569)

เจ้าของงานตัดสินใจ **ยืน 182 ตามที่แสดงตอนกดรับ** ด้วยเหตุผลว่าเป็นข้อผูกพัน
แก้ที่ `jobs/-P0QekIHDPBe7phJhabu/rider_fee` = 182 ก่อนกด APPROVE จึงเหลือแถวเดียวใน ledger
ไม่ต้องมีแถวแก้ตาม · **ไม่แตะ `rider_fee_meta`** ปล่อยให้บันทึกตามจริงว่าการคำนวณได้ 157
จาก 19.35 กม. — เลขนั้นคือหลักฐาน ส่วน 182 คือการตัดสินใจที่ทับมัน สองอย่างต้องอยู่คู่กัน

ยืนยันจากโค้ดว่าไม่มี trigger ตัวไหนเขียนทับ: `onJobHandedOverCalcRiderFee` ข้ามทันทีถ้า
`rider_fee > 0` (`bkk-system/functions/index.js:3419-3425`) · `onRiderAssignedRecalcEstimate`
แตะเฉพาะ `rider_fee_estimate` · มีแต่คำแย้งหมุดที่เขียนทับได้ ซึ่งต้องมีคนยื่น

---

## ขนาดของผลกระทบจริง — อ่านก่อนใช้ตัวเลขในรายงานนี้ไปตัดสินใจ

> **เพิ่ม 2 ก.ย. 2569 หลังรันสคริปต์สแกนกับ production**
>
> `bkk-system/scripts/audit-travel-mode-repricing.cjs` พบ **50 ใบ** ที่ฐานคิดเงินตอนจบงาน
> ไม่ตรงกับตอนกดรับ ยอดรวมที่ไรเดอร์เสียดูเหมือน 1,366 บาท
>
> **แต่ 49 ใน 50 ใบเป็นงานทดสอบของเจ้าของกิจการเอง** — ไรเดอร์
> `GmxKmv51QxNr0HTuZ5FqmIB50kQ2` คือเจ้าของ ยังไม่เคยเบิกใช้จริง
> มีใบเดียวที่เป็นลูกจ้างจริง (`LALYFeWzdZQKP8gfrbPA4MMDT6e2`, งาน `OID-MTIAI3FH-851`
> ซึ่งเป็นเคสของรายงานฉบับนี้พอดี)
>
> **หนี้จริงที่ค้างกับคนอื่น = 25 บาท ใบเดียว และจ่ายไปแล้ว**
> ห้ามอ่านตัวเลข 1,366 ว่าเป็นเงินค้างจ่ายไรเดอร์ และห้ามเอา "8 ใบยังไม่จ่าย −585"
> ไปตั้งเป็นงานตามเก็บ — เจ้าของตัดสินแล้วว่าไม่ต้องแก้ 7 ใบที่เหลือ

**แต่ปรากฏการณ์เป็นของจริงทุกตัวเลข** และนี่คือส่วนที่ต้องเอาไปใช้ต่อ:

| | ถัง A (config เปลี่ยน) | ถัง C (ระยะทางแกว่ง) |
|---|---|---|
| จำนวน | 3 ใบ | 46 ใบ |
| ทิศทาง | เสีย 3 · ได้เพิ่ม **0** | เสีย 18 · ได้เพิ่ม **18** · เท่าเดิม 10 |
| ผลรวมสุทธิ | −365 | −101 (แทบเป็นศูนย์) |
| คลาดเคลื่อนเฉลี่ยต่อใบ | 91 บาท | **41 บาท** |
| ระยะทางเปลี่ยนเฉลี่ย | — | **18.4%** สูงสุด 51% |

- **ถัง A เอียงข้างเดียว** = มีคนไปเปลี่ยนค่า (F11) เกิดสองครั้ง: rate card ย้ายจาก
  `60/15` เป็น `100/5` เมื่อ **22 ส.ค.** และ `travel_mode` สลับเมื่อ **1 ก.ย.**
- **ถัง C สมมาตร** = ความคลาดเคลื่อนของการวัด ไม่ใช่การจ่ายน้อย — **แต่นั่นแย่ในแบบของมันเอง**
  ไรเดอร์ถูกสัญญา X แล้วได้ X ± 41 บาทแบบสุ่ม ซึ่งทำลายความเชื่อใจไม่ว่าทิศไหน
  และ 18 ใบที่ได้เพิ่มก็เป็นต้นทุนที่ไม่ได้วางแผน
- **18.4% ใหญ่เกินกว่าจราจรจะอธิบาย** — สมมติฐานอันดับหนึ่งคือ `resolveBranchCoords`
  มี fallback สามชั้น สองรอบอาจตกคนละชั้นแล้ววัดไปคนละสาขา ยังพิสูจน์ไม่ได้เพราะ
  `branch_source` เพิ่งถูกเพิ่มเข้า meta (สคริปต์รายงานฟิลด์นี้แล้ว งานใหม่จะตรวจได้)

**ประโยคที่ถูกต้องสำหรับคนอ่านรอบหน้า:** กลไกนี้ทำงานผิดพลาดมา 4 เดือนโดยไม่มีใครเจ็บ
เพราะมันยิงใส่เจ้าของเอง — และเพิ่งถูกเล็งไปที่ลูกจ้างจริงเป็นครั้งแรกเมื่อ 1 ก.ย. 2569
ความเร่งด่วนจึงไม่ได้มาจากยอดเงินที่ค้าง แต่มาจากการที่กองไรเดอร์กำลังจะโตขึ้น
