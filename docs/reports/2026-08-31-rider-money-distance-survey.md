...
2. **wallet ไรเดอร์นับ `LOGISTICS_REVENUE` เป็นเงินเข้าของตัวเอง** —
   ตอน finance จ่ายลูกค้า มีการเขียน CREDIT `category:'LOGISTICS_REVENUE'` ด้วย `rider_id: selectedTx.rider_id` ยอด = `rider_fee` (bkk-system/src/pages/finance/components/TradeInPayouts.tsx:170-182) ขณะที่ balance ฝั่งแอป rider รวม**ทุก CREDIT ที่ rider_id ตรง โดยไม่กรอง category** (bkk-rider-app/src/hooks/useRiderData.ts:153-154) → งาน Pickup ที่จ่ายลูกค้าแล้ว+settle แล้ว ไรเดอร์เห็นเงินเข้า **สองก้อน** (LOGISTICS_REVENUE + JOB_PAYOUT) = balance เกินจริงเท่ากับ rider_fee ต่องาน และแถวโผล่ใน WalletTab ด้วยชื่อ category ดิบ (WalletTab.tsx:45,50)
...
1. **การถอนเงินของไรเดอร์ทุกวันนี้ทำงานอยู่จริงไหม?** ((ค)#1) — ไรเดอร์เคยกด "ยืนยันการถอนเงิน" สำเร็จหรือขึ้น error? และแถว `type:'Withdrawal'` ที่หน้า finance เห็น ถูกสร้างจากช่องทางไหน (แอดมินสร้างมือ / แอปเวอร์ชันเก่า / ไม่มีเลย)? คำตอบชี้ว่าต้องซ่อมฝั่งเขียน (rules+path) หรือฝั่งอ่าน
2. **`LOGISTICS_REVENUE` ตั้งใจให้เป็นเงินของใคร?** ((ค)#2) — มันคือรายได้ค่าบริการของบริษัท (ควร `rider_id:'SYSTEM'`) หรือเป็นเงินไรเดอร์จริง (แล้ว JOB_PAYOUT ต้องไม่จ่ายซ้ำ)? มีข้อมูล production ที่ balance ไรเดอร์บวมจากคู่ transaction นี้แล้วหรือยัง?
