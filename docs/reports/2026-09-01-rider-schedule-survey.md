...
> ฝั่งไรเดอร์ ละเอียดกว่าหัวข้อ 3 ที่นี่) และ
...
>
> **หมายเหตุเรื่องเอกสาร employees:** โจทย์ให้อ้าง
> `docs/reports/2026-09-01-employee-lifecycle-survey.md` — **ไม่พบไฟล์นี้ในทั้ง 3 repo**
> (`find . -iname "*employee*"` = 0 แถว; docs/reports ที่มีอยู่มี 6 ไฟล์ ไม่มีตัวนี้)
> คอลัมน์ "ผูกกับ employees" ด้านล่างจึงเขียนจาก**ของที่มีจริงในโค้ด** — โครงสร้าง
> `riders/{id}` (`employment.type`/`tax_id`/`tax_address` ที่
> `bkk-system/src/pages/fleet/RiderManagement.tsx:223-227`, `vehicle_type` ที่ `:217-219`,
> `approval_status`, `bank`, `documents`) และตาราง `staff` ที่แยกกันอยู่คนละโหนด — **ต้อง
> re-check กับเอกสาร employee lifecycle เมื่อหาไฟล์เจอ ก่อนใช้ข้อสรุปคอลัมน์นี้**

| # | ช่องว่าง | ชนิด | status model | ผูกกับ employees |
|---|---|---|---|---|
| 1 | **ไม่มี custody chain ตอนส่งมอบเครื่อง** — `Pending QC` ไรเดอร์กดเอง ไม่มี QR ไม่มีสแกน ไม่มีฝั่งสาขายืนยัน มีแค่ GPS 300 ม. แบบ non-blocking (5.1) | **ไม่มีเลย** | **ใช่ — ต้องออกแบบร่วม** ต้องเพิ่มสถานะ/สอง-เฟส (`Handover Pending` → `Received`) ซึ่งแตะ `job-statuses.ts` ทั้ง 3 repo + CI | ผู้รับต้องเป็น**พนักงานที่ระบุตัวได้** — ปลายทางของ audit คือ `staff` ไม่ใช่ `riders` |
...
| 3 | **สิทธิ์อ่าน/เขียนงานฝั่งไรเดอร์กว้างกว่าที่ UI แสดง** — `.read` ทั้งโหนด `jobs` และ `.write` ได้ทุกงานที่ `rider_id` ว่างโดยไม่ตรวจ status (4.1) | **มีแต่พึ่ง client** | ใช่บางส่วน — rule ที่จะตรวจ status ได้ต้องมีชุด status ที่นิ่งก่อน | ผูกโดยตรง: rule ตัดสินจากการมีแถวใน `/riders` ถ้า employees ถูกรวมศูนย์ ต้องแก้ rule ด้วย |
...
| 5 | **ไม่มี shift / on-duty ที่ persist** — สวิตช์รับงานอยู่ใน memory, `riders/{id}/status` ไม่เคยถูกตั้งเป็น Offline, ไม่มี log ชั่วโมงทำงาน (4.4) | **ไม่มีเลย** | ไม่ | **ผูกหนักที่สุด** — ตารางเวร/ชั่วโมงทำงานคือของ employee ไม่ใช่ของ job; ถ้า employees จะโตจาก `riders` ต้องออกแบบพร้อมกันตั้งแต่แรก |
...
| 11 | **ไม่มีหน้าไหนตอบ "วันนี้วิ่งกี่งาน กี่กม. ได้เงินเท่าไหร่"** ครบในที่เดียว และ `HistoryTab` ใช้ `|| 150` แทน `getRiderPayout()` (6.1) | **มีแต่กระจัดกระจาย + มีบั๊ก** | ไม่ | ผูก: รายงานค่าตอบแทนย้อนหลังเป็นอินพุตของ payroll/WHT ซึ่งอยู่ฝั่ง employee |
